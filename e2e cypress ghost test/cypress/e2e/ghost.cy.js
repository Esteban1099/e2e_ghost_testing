...
function desingPageEr(type, pageName, pageText, tagName) {
...
function checkPublishPageTag(position, tag) {
  var contador = 0;
  cy.get('ol.gh-list').then($ol => {
    var objectList = $ol.get(0);
    var items = objectList.querySelectorAll('li.gh-posts-list-item')
...
describe('E2E Test in ghost', () => {
  ...
  it('Feature: Create page - Scenario: Create page publish and edit to set tag', () => {
    ...
    cy.wait(1000);
    // And I desing page title, text and publish 
    desingPageEr('basic-text-tag', pageName, pageText, tagName)
    // And I wait 1 seconds
    cy.wait(1000);
    // And I navigate to pages
    navigateModule('pages')
